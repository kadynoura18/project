...
const SigninPrestataire = () => {
  const [nom, setNom] = useState('');
    ...
    const { setPrestataireId } = useContext(PrestataireContext);

    const intitules = [
      "Organisateur de Mariage (Wedding Planner)",
      "Organisateur de Baptême (Baptism Planner)",
      "Organisateur d'anniversaire (Birth Planner)",
      "Maquilleuse",
      "Location de voiture",
      "Service Traiteurs",
      "Créateur de souvenir",
      "Tenue",
      "Animation"
    ];
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        const response = await AuthService.registerPrestataire(nom, prenom, email, mdp, intitule, recaptchaValue, photo);

        // Vérifiez que la réponse contient les données attendues
        console.log('Réponse brute de l\'API:', response);
        if (response) {
          console.log('Contenu de response:', response);
          const prestataireId = response; // La réponse est un entier représentant l'ID du prestataire
          if (prestataireId) {
 ...
 const handleGoogleLogin = async (googleUser) => {
  ...
  const response = await axios.post('http://localhost:8080/api/v1/rest/prestataires/inscription', { token });
  ...
  <FormLabel>Choisir son intitulé</FormLabel>
  <Form.Control
    as="select"
    value={intitule}
    ...
    style={{ width: '100%', height: '40px', borderRadius: '5px', boxShadow: '0px 0px 5px rgba(0,0,0,0.1)' }}
  >
    <option disabled value="">Choisir son intitulé</option>
    {intitules.map((intitule, index) => (
      <option key={index} value={intitule}>
        {intitule}
      </option>
    ))}
  </Form.Control>
...
export default SigninPrestataire;
